...
/**
 * Standard component definitions for json-render design system packages.
 *
 * These are the abstract, design-system-agnostic catalog definitions that all
 * design system packages (@json-render/shadcn, @json-render/mui, etc.) share.
 * Each package re-exports these under its own name and provides platform-specific
 * component implementations (registries).
 *
 * Specs generated against these definitions work with ANY design system registry.
 */
...
export const validateOnSchema = z.enum(["change", "blur", "submit"]).nullable();
...
export const standardComponentDefinitions = {
  ...
  Drawer: {
    ...
    slots: ["default"],
    description:
      "Side/bottom drawer. Set openPath to a boolean state path. Use setState to toggle.",
  },
...
/**
 * Infer the props type for a standard component by name.
 */
export type StandardProps<K extends keyof typeof standardComponentDefinitions> =
  z.output<(typeof standardComponentDefinitions)[K]["props"]>;
